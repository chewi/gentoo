...
IUSE="ftp gd imagemagick mssql mysqli postgres sqlite zlib"
...
src_install() {
	...
	webapp_serverowned "${MY_HTDOCSDIR}"/{cache,config.php,files,store}
	webapp_serverowned "${MY_HTDOCSDIR}"/images/avatars/upload
	webapp_configfile  "${MY_HTDOCSDIR}"/config.php

	webapp_hook_script "${FILESDIR}"/reconfig
	webapp_postinst_txt en "${FILESDIR}"/postinstall-en.txt
	webapp_server_configfile nginx "${FILESDIR}"/nginx.conf main.conf
	webapp_src_install
